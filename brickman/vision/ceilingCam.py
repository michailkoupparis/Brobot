...
class ceilingCam():
    ...
    def detectRed(self, img, size):
        ...
        for cnt in contours:
            ...
            if radius > size and self.insideRoom(x,y):
                center = (int(x),int(y))
                radius = int(radius)
                cv2.putText(img, 'red',center,cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    def detectBlue(self, img, size):
        ...
        for cnt in contours:
            ...
            if radius > size and self.insideRoom(x,y):
                center = (int(x),int(y))
                radius = int(radius)
                cv2.putText(img, 'blue',center,cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                positions.append((x,y))
    ...
    def detectGreen(self, img, size):
        ...
        for cnt in contours:
            ...
            if radius > size and self.insideRoom(x,y):
                center = (int(x),int(y))
                radius = int(radius)
                cv2.putText(img, 'green',center,cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                positions.append((x,y))
    ...
    def detectYellow(self, img, size):
        ...
        for cnt in contours:
            ...
            if radius > size and self.insideRoom(x,y):
                center = (int(x),int(y))
                radius = int(radius)
                cv2.putText(img, 'yellow',center,cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                positions.append((x,y))
    ...
    def go(self, img):
        ...
        _, positions_blue = self.detectBlue(img, 10)

        # get all green object positions
        _, positions_green = self.detectGreen(img, 10)
        ...
        if center_front is not None and len(positions) >= 1:
            ...
            cv2.putText(img, "angle from object: {}, angle from home: {}, distance from object: {}"\
                        .format(angle_obj,angle_home,distance_object),
...
if __name__ == '__main__':
    ...
    camera = ceilingCam(40,550,80,450)d
